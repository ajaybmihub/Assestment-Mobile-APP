...
export default function TicketDetailPage() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: '1rem' }}>
       ...
       <div style={{ color: 'var(--text-3)', fontSize: '0.9rem', fontWeight: 500, letterSpacing: '0.05em' }}>FETCHING TICKET...</div>
  ...
  if (!ticket) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '80vh', textAlign: 'center' }}>
      ...
      <Link href="/tickets" className="badge badge-purple" style={{ padding: '0.75rem 1.5rem', textDecoration: 'none' }}>Return to Support Tickets</Link>
    </div>
  );
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '0 1rem' }}>
      <div className="page-header" style={{ marginBottom: '2.5rem' }}>
        <div style={{ display: 'flex', gap: '0.5rem', fontSize: '0.8rem', color: 'var(--text-4)', marginBottom: '1.5rem' }}>
          ...
          <span>/</span>
          <span style={{ color: 'var(--accent-light)', fontWeight: 600 }}>TKT-{ticket._id.slice(-6).toUpperCase()}</span>
        </div>
        <div className="page-header-row">
          <div>
            <div className="page-title" style={{ fontSize: '2.2rem', letterSpacing: '-0.02em', marginBottom: '0.5rem' }}>{ticket.title}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
               <span style={{ fontSize: '0.65rem', fontWeight: 800, color: 'var(--accent-light)', background: 'var(--accent-subtle)', padding: '4px 10px', borderRadius: '6px', textTransform: 'uppercase' }}>
                 {ticket.category}
               </span>
          ...
          <div style={{ textAlign: 'right' }}>
             ...
             <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(30,32,44,0.5)', padding: '10px 20px', borderRadius: '12px', border: '1px solid var(--border-subtle)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        <div className="card" style={{ padding: '2.5rem', background: 'rgba(30, 32, 44, 0.4)', border: '1px solid var(--border-subtle)', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}>
           <div style={{ fontSize: '0.7rem', fontWeight: 800, color: 'var(--accent-light)', textTransform: 'uppercase', marginBottom: '1.5rem', letterSpacing: '0.1em' }}>Issue Description</div>
           <div style={{ fontSize: '1.15rem', color: 'var(--text-2)', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>
             {ticket.description}
           </div>
        </div>

        <div className="card" style={{ border: '1px solid var(--accent-subtle)', background: 'linear-gradient(180deg, rgba(30,41,59,0.2) 0%, rgba(15,23,42,0.4) 100%)' }}>
          <div className="card-header" style={{ padding: '1.5rem 2rem', borderBottom: '1px solid var(--border-subtle)' }}>
            <div className="card-title" style={{ fontSize: '1rem', color: 'var(--accent-light)' }}>Action Center</div>
          </div>
          <div style={{ padding: '2.5rem' }}>
             ...
             <textarea 
                ...
                placeholder="Ex: Fixed the API timeout issue by optimizing the query..."
                style={{ 
                  width: '100%', 
                  minHeight: '140px', 
             ...
             <div style={{ display: 'flex', gap: '1.5rem' }}>
                <button 
                  ...
                  style={{ flex: 1, height: '54px', fontSize: '0.9rem', fontWeight: 700, borderRadius: '12px' }}
                >
        ...
        <div style={{ textAlign: 'center', marginTop: '2rem' }}>
           <Link href="/tickets" style={{ color: 'var(--text-4)', fontSize: '0.9rem', textDecoration: 'none' }}>
             ← Back to all tickets
           </Link>
        </div>
      </div>
